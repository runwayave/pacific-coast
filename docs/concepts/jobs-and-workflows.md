...
workflow OnboardVendor in vendor {
  ...
  step setup_integration {
    job ShopifyImport
    args { vendor_id: $vendor_id, import_strategy: "replace" }
  }
...
1. A caller submits a job via `SubmitJob` (with the generated `ShopifyImportArgs` struct marshaled to JSON) or a procedure's `enqueue` step. The server INSERTs a row into `atlantis.jobs`.
...
```go
vendorpkg.RegisterShopifyImport(reg, &myShopifyHandler{client: shopifyClient})
```
