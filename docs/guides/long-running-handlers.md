...
- External calls (Stripe charge, Shopify product create, S3 upload, email send) need an idempotency key the external system honors, or a read-then-write against its state.
...
CREATE TABLE shopify_import_runs (
  vendor_id   varchar(7) PRIMARY KEY,
...
```go
err := tx.QueryRow(ctx, `
    INSERT INTO shopify_import_runs (vendor_id, status)
    VALUES ($1, 'running')
    ON CONFLICT (vendor_id) DO UPDATE SET status = 'running'
    RETURNING next_cursor`, args.VendorId).Scan(&cursor)
```
...
Override per-job with the `heartbeat` modifier when a handler blocks on a single external call longer than the default — a Shopify catalog page that takes 8 minutes, an ML training step that takes 20 minutes. Without the override the dispatcher will revoke and re-dispatch mid-call even though the worker is alive.
...
## 5. Worked example — Shopify catalog import

A handler that fetches a vendor's full Shopify catalog. The Admin API paginates at ~25 products per request; each request takes 1–3 seconds; a large catalog has tens of thousands of products. A typical run is 8–15 minutes; the tail blows past an hour.
...
```sql
CREATE TABLE vendor_shopify_import_state (
  vendor_id     varchar(7) PRIMARY KEY,
  status        text NOT NULL,         -- 'running' | 'complete' | 'failed'
  next_cursor   text,                  -- Shopify page_info token
  pages_done    int NOT NULL DEFAULT 0,
  products_done int NOT NULL DEFAULT 0,
  started_at    timestamptz NOT NULL DEFAULT now(),
  updated_at    timestamptz NOT NULL DEFAULT now()
);
```
...
func (h *shopifyImportHandler) Handle(ctx context.Context, args vendorpkg.ShopifyImportArgs) error {
    ...
    err := h.db.QueryRow(ctx, `
        INSERT INTO vendor_shopify_import_state (vendor_id, status)
        VALUES ($1, 'running')
        ON CONFLICT (vendor_id) DO UPDATE SET status = 'running', updated_at = now()
        RETURNING next_cursor, pages_done, products_done`,
        args.VendorId,
    ).Scan(&state.cursor, &state.pages, &state.products)
    if err != nil {
        return fmt.Errorf("open import state: %w", err)
    }
    _ = jobs.Checkpoint(ctx, 0, fmt.Sprintf("resuming pages=%d products=%d", state.pages, state.products))

    // 2. Paginated fetch + write. Each iteration is one Shopify page.
    for {
        page, err := h.client.FetchProducts(ctx, args.VendorId, state.cursor)
        if err != nil {
            return fmt.Errorf("fetch page after cursor=%q: %w", state.cursor, err)
        }
        if err := h.writeProducts(ctx, args.VendorId, page.Products); err != nil {
        ...
        state.pages++
        state.products += len(page.Products)
        _, err = h.db.Exec(ctx, `
            UPDATE vendor_shopify_import_state
               SET next_cursor = $1, pages_done = $2, products_done = $3, updated_at = now()
             WHERE vendor_id = $4`,
            state.cursor, state.pages, state.products, args.VendorId)
        if err != nil {
        ...
        _ = jobs.Checkpoint(ctx,
            percentDone(state.products, page.TotalEstimate),
            fmt.Sprintf("page=%d products=%d", state.pages, state.products))
